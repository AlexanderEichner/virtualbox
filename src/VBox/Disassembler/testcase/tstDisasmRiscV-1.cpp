/* $Id$ */
/** @file
 * VBox disassembler - Testcase for RISC-V
 */
...
#include "tstDisasmRiscV-1-tests.h"
...
DECLASM(int) TestProcRiscV(void);
DECLASM(int) TestProcRiscV_EndProc(void);
...
static const RTSCRIPTLEXTOKMATCH s_aMatches[] =
{
    ...
    { RT_STR_TUPLE("<iprt/asmdefs-riscv.h>"),       RTSCRIPTLEXTOKTYPE_KEYWORD,    true,  0 },
    ...
    { RT_STR_TUPLE("testprocriscv"),                RTSCRIPTLEXTOKTYPE_KEYWORD,    true,  0 },
    { RT_STR_TUPLE("testprocriscv_endproc"),        RTSCRIPTLEXTOKTYPE_KEYWORD,    true,  0 },
    { RT_STR_TUPLE(".option"),                      RTSCRIPTLEXTOKTYPE_KEYWORD,    true,  0 },
    { RT_STR_TUPLE("norvc"),                        RTSCRIPTLEXTOKTYPE_KEYWORD,    true,  0 },
...
{
    { '0',  '9',   RTSCRIPT_LEX_RULE_DEFAULT, rtScriptLexParseNumber,             NULL},
    { '+',  '+',   RTSCRIPT_LEX_RULE_DEFAULT, rtScriptLexParseNumber,             NULL},
    { '-',  '-',   RTSCRIPT_LEX_RULE_DEFAULT, rtScriptLexParseNumber,             NULL},
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            switch (enmDisCpuMode)
            {
                ...
                case DISCPUMODE_RISCV_RV32:
                    cch = DISFormatRiscVEx(&Dis, &szOutput[0], sizeof(szOutput),
            ...
            if (cErrBefore != RTTestIErrorCount())
            {
                ...
                DISFormatRiscVEx(&Dis, szOutput, sizeof(szOutput),
...
static void testDisasComplianceAgaistCapstone(void)
{
    ...
    cs_err rcCs    = cs_open(CS_ARCH_RISCV, CS_MODE_LITTLE_ENDIAN, &hDisasm);
    ...
    for (uint32_t u32Insn = 0; u32Insn < UINT32_MAX; u32Insn++)
    {
         ...
         * which would trip the semantic matching later on.
         */
        int rc = DISInstrEx((uintptr_t)&u32Insn, DISCPUMODE_RISCV_RV64, DISOPTYPE_ALL,
                            NULL /*pfnReadBytes*/, NULL /*pvUser*/, &Dis, &cb);
        ...
        else
        {
            ...
            size_t cch = DISFormatRiscVEx(&Dis, &szOutput[0], sizeof(szOutput),
                                          DIS_FMT_FLAGS_RELATIVE_BRANCH,
                                          NULL /*pfnGetSymbol*/, NULL /*pvUser*/);
            Assert(cch);
...
int main(int argc, char **argv)
{
    ...
    {
        { "64-bit",      (uint8_t const *)(uintptr_t)TestProcRiscV, (uintptr_t)&TestProcRiscV_EndProc, DISCPUMODE_RISCV_RV64,
          g_abtstDisasmRiscV_1, g_cbtstDisasmRiscV_1 },
    };
