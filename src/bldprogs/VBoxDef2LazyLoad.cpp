/* $Id: VBoxDef2LazyLoad.cpp 114564 2026-06-30 10:57:17Z knut.osmundsen@oracle.com $ */
/** @file
...
static RTLDRARCH const  g_enmTargetDefault = RTLDRARCH_RISCV64;
#else
# error "Port me!"
#endif
...
/**
 * Generates the assembly source code for RV64I, writing it
 * to @a pOutput.
 *
 * @returns RTEXITCODE_SUCCESS or RTEXITCODE_FAILURE, in the latter case full
 *          details has been displayed.
 ...
 *                               when closing).
 */
static RTEXITCODE generateOutputInnerRiscv64(FILE *pOutput)
{
    fprintf(pOutput, "/*\n");
    for (unsigned i = 0; i < g_cInputs; i++)
        fprintf(pOutput, " * Autogenerated from '%s'.\n", g_apszInputs[i]);
    ...
    fprintf(pOutput,
            ...
            "\n"
            "\n"
            "#include \"iprt/asmdefs-riscv.h\"\n"
            "\n"
            "/* No compressed instructions to be able to fit relocations. */\n"
            ".option norvc\n"
            "\n"
            "\n");
    ...
    for (PMYEXPORT pExp = g_pExpHead; pExp; pExp = pExp->pNext)
        if (!pExp->fData)
            ...
            fprintf(pOutput,
                    ...
                    ".globl NAME(LazyGetPtr_%s)\n"
                    "NAME(LazyGetPtr_%s):\n"
                    "    lla     t0, NAME(g_LazyPtr_%s)\n"
                    "    ld      t0, 0(t0)\n"
                    "    beqz    t0, ___LazyLoad___%s\n"
                    "    mv      a0, t0\n"
                    "    ret\n",
                    pExp->pszExportedNm, pExp->pszExportedNm,
                    pExp->pszExportedNm,
    ...
    fprintf(pOutput,
            ...
            "    sd      s3, 88(sp)\n"
            "\n"
            "    /* Shift the symbol name to s2 and g_pfnXXXX pointer to s3 as these are preserved registers\n"
            "     * (in case we need to call LazyLoadModule/RTLdrLoad) */\n"
            "    mv      s2, t0\n"
            "    mv      s3, t1\n"
            "\n"
            ...
            "    lla     a2, g_szLibrary\n"
            "    mv      a1, s2\n"
            "    lla     a0, g_szFailLoadFmt\n"
            "    sd      a1,  (sp)\n"
            "    sd      a2,  8(sp)\n"
            "    sd      a3, 16(sp)\n"
            "    call    NAME(RTAssertMsg2Weak)@plt\n"
            "Lbadsymloop:\n"
            "    ebreak\n"
            "    j       Lbadsymloop\n"
    ...
    if (!g_fSystemLibrary)
        fprintf(pOutput,
                ...
                "    lla     a1, g_hMod\n"
                "    lla     a0, g_szLibrary\n"
                "    call    NAME(SUPR3HardenedLdrLoadAppPriv)@plt\n");
    else
        fprintf(pOutput,
                "    /* Call RTLdrLoadSystem(const char *pszFilename, bool fNoUnload, PRTLDRMOD phLdrMod); */\n"
                "    lla     a2, g_hMod\n"
                "    lla     a0, g_szLibrary\n"
                "    li      a1, 1\n"
                "    call    NAME(RTLdrLoadSystem)@plt\n");

    fprintf(pOutput,
            "    beqz    a0, Lload_return\n"
            "\n"
            "Lbadload: /* Call sRTAssertMsg2Weak. Variadic (...) arguments are passed on the stack it seems. */\n"
            "    mv      a2, a0\n"
            "    lla     a1, g_szLibrary\n"
            "    lla     a0, g_szFailResolveFmt\n"
            "    /**@todo*/\n"
            "    call    NAME(RTAssertMsg2Weak)@plt\n"
            "Lbadloadloop:\n"
            "    ebreak\n"
    ...
    if (pOutput)
    {
        switch (g_enmTarget)
        {
            ...
            case RTLDRARCH_RISCV64:
                rcExit = generateOutputInnerRiscv64(pOutput);
                break;
            default:
                rcExit = RTEXITCODE_FAILURE;
                break;
...
static int usage(const char *pszArgv0)
{
    ...
    printf("VBoxDef2LazyLoad - Lazy DLL/SO/DYLIB loader code generator.\n"
           ...
           "    arm64, riscv64 and default.  Empty string is an alias for default.\n"
